...
def test_invalid_authorization_code_keeps_state_unconsumed_and_returns_safe_error(client, monkeypatch):
    _, params = start_authorization(client)
    ...
    with SessionLocal() as db:
        assert db.query(OAuthState).first().consumed is False
        assert db.query(OAuthIntegration).count() == 0

def test_invalid_token_response_rolls_back_integration_and_state(client, monkeypatch):
    _, params = start_authorization(client)
    invalid_payload = {
        "access_token": "access-token-123",
        "token_type": "bearer",
    ...
    monkeypatch.setattr(
        "app.integrations.mercadolivre.service.MercadoLivreHttpClient",
        lambda: FakeMercadoLivreClient(token_payload=invalid_payload),
    )

    response = client.get(f"/oauth/mercadolivre/callback?code=auth-code&state={params['state']}")

    assert response.status_code == 400
    assert response.json()["detail"] == "Mercado Livre OAuth response did not include a refresh token"
    with SessionLocal() as db:
        assert db.query(OAuthState).first().consumed is False
        assert db.query(OAuthIntegration).count() == 0
