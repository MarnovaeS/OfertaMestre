...
class MercadoLivreCollectorService:
    ...
    def fetch_normalized_item(self, item_id: str) -> ExternalOfferInput:
        ...
        sale_price = self._get_sale_price_if_allowed(item_id, access_token)
    ...
    def _get_sale_price_if_allowed(self, item_id: str, access_token: str) -> dict[str, Any] | None:
        try:
            return self.client.get_sale_price(item_id, access_token)
        except MercadoLivreForbiddenError as exc:
            logger.warning(
                "Mercado Livre complementary request failed",
                extra={"operation": exc.operation or "sale_price", "item_id": item_id},
            )
            return None
    ...
    def _get_seller_if_needed(self, item: dict[str, Any], access_token: str) -> dict[str, Any] | None:
        ...
        if cache_key not in self._seller_cache:
            try:
                ...
                logger.warning(
                    "Mercado Livre complementary request failed",
                    extra={"operation": exc.operation or "seller", "seller_id": cache_key},
                )
                return None
